...
transformed parameters {
  ...
  for (t in (max_Lag+1):N_time) {
    alpha_MarSun[t] = mu[t] + b_temp[t] * temp_MarSun[t-Lag_temp] + b_light[t] * light_MarSun[t-Lag_light] + b_CCA1[t] * CCA1_MarSun[t-Lag_CCA1];
  }
  for (t in (max_Lag+1):N_time) {
    alpha_MarShade[t] = mu[t] + b_temp[t] * temp_MarShade[t-Lag_temp] + b_light[t] * light_MarShade[t-Lag_light] + b_CCA1[t] * CCA1_MarShade[t-Lag_CCA1];
  }
  for (t in (max_Lag+1):N_time) {
    alpha_SepSun[t] = mu[t] + b_temp[t] * temp_SepSun[t-Lag_temp] + b_light[t] * light_SepSun[t-Lag_light] + b_CCA1[t] * CCA1_SepSun[t-Lag_CCA1];
  }
  for (t in (max_Lag+1):N_time) {
    alpha_SepShade[t] = mu[t] + b_temp[t] * temp_SepShade[t-Lag_temp] + b_light[t] * light_SepShade[t-Lag_light] + b_CCA1[t] * CCA1_SepShade[t-Lag_CCA1];
  }
}
...
model {
  ...
  if (max_Lag == 0) {
    for (t in 2:N_time) {
      ...
      b_CCA1[t] ~ normal(b_CCA1[t-1], s_b_CCA1);
    }
  } else {
    for (t in (max_Lag+1):N_time) {
      mu[t] ~ normal(mu[t-1], s_mu);
