...
## Load stan model
model <- cmdstan_model("stan_model/SSM_regression_Sep_SIG5_muCommon_LagTempLight0_WAIC_rev2.stan")
...
for(i in 1:6){
  data_list <- list(
    N_time = length(unique(data_SepSun$Time)),
  ...
  fit <- model$sample(
    data = data_list,
    init = function() { list(alpha_SepSun = as.numeric(tapply(data_SepSun$SIG5, data_SepSun$Time, mean, na.rm=T)),
    ...
    sig_figs = 4,
    output_dir = "/tmp",
    output_basename = paste0("Sep_SIG5_WAIC_", i)
  )
  
  # 99% Bayesian credible intervals
  outcsv_name <- list.files("/tmp")
  outcsv_name <- outcsv_name[grep(paste0("Sep_SIG5_WAIC_", i), outcsv_name)]
  tmp_csv_b_temp <- NULL
  ...
  for(j in 1:length(outcsv_name)){
    ...
    tmp_csv_b_CCA1 <- rbind(tmp_csv_b_CCA1, tmp_csv[,str_starts(names(tmp_csv), "b_CCA1")])
  ...
  df_b_CCA1 <- as.data.frame(round(t(apply(tmp_csv_b_CCA1, 2, quantile99)), digits = 4))
  ...
  df_all <- rbind(df_b_temp, df_b_light, df_b_CCA1, df_alpha_SepSun, df_alpha_SepShade)
  df_all <- df_all %>%
    mutate(par = row.names(df_all)) %>%
    relocate(par)
  
  fwrite(df_all,
         file = paste0("SSM_out/SSM_regression_Sep_SIG5_muCommon_out_", 
                       "Lagtemp", data_list$Lag_temp, 
                       "_Laglight", data_list$Lag_light, 
                       "_LagCCA1", data_list$Lag_CCA1, "_WAIC_rev2_99.csv"))
  
  # MCMC samples of alpha
  tmp_csv_alpha <- cbind(tmp_csv_alpha_SepSun, tmp_csv_alpha_SepShade)
  fwrite(tmp_csv_alpha,
         file = paste0("SSM_out/SSM_regression_Sep_SIG5_muCommon_out_", 
                       "Lagtemp", data_list$Lag_temp, 
                       "_Laglight", data_list$Lag_light, 
                       "_LagCCA1", data_list$Lag_CCA1, "_WAIC_rev2_MCMCalpha.csv"))
  
  # log likelihood
  df_log_lik <- as.data.frame(cbind(tmp_csv_log_lik_SepSun, tmp_csv_log_lik_SepShade))
  
  fwrite(df_log_lik,
         file = paste0("SSM_out/SSM_regression_Sep_SIG5_muCommon_out_", 
                       "Lagtemp", data_list$Lag_temp, 
                       "_Laglight", data_list$Lag_light, 
                       "_LagCCA1", data_list$Lag_CCA1, "_WAIC_rev2_loglik.csv"))
