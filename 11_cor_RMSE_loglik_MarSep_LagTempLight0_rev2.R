...
df_cor_rmse_loglik_SIG5 <- cbind(Lag_env_SIG5, 
...
df_cor_rmse_loglik_BLRP <- cbind(Lag_env_BLRP, 
...
for(i in 1:nrow(Lag_env_SIG5)){
  ...
  # Load mcmc samples
  df_mcmc <- as.data.frame(fread(paste0("SSM_out/SSM_regression_MarSep_SIG5_muCommon_out_", 
  ...
  obs <- c(SIG5_MarSun$Mean_SIG5, SIG5_MarShade$Mean_SIG5, 
           SIG5_SepSun$Mean_SIG5, SIG5_SepShade$Mean_SIG5)
  for(j in 1:nrow(df_mcmc)){
    cor_mcmc_obs[j] <- cor(as.numeric(df_mcmc[j,use_idx]), obs[use_idx])
  }
  df_cor_rmse_loglik_SIG5[i,4:6] <- quantile99(cor_mcmc_obs)[2:4]
  df_cor_rmse_loglik_SIG5[i,7] <- mean(cor_mcmc_obs)
  ...
  df_cor_rmse_loglik_SIG5[i,8:10] <- quantile99(rmse_mcmc_obs)[2:4]
  df_cor_rmse_loglik_SIG5[i,11] <- mean(rmse_mcmc_obs)
  
  # Log likelihood
  df_log_lik <- as.data.frame(fread(paste0("SSM_out/SSM_regression_MarSep_SIG5_muCommon_out_", 
                                           "Lagtemp", Lag_env_SIG5$Lag_temp[i],
                                           "_Laglight", Lag_env_SIG5$Lag_light[i],
                                           "_LagCCA1", Lag_env_SIG5$Lag_CCA1[i],
                                           "_WAIC_rev2_loglik.csv")))
  df_log_lik <- df_log_lik %>% select_if(negate(anyNA))
  df_cor_rmse_loglik_SIG5[i,12:14] <- quantile99(rowMeans(df_log_lik))[2:4]
  df_cor_rmse_loglik_SIG5[i,15] <- mean(rowMeans(df_log_lik))
}
...
for(i in 1:nrow(Lag_env_BLRP)){
  # Load mcmc samples
  df_mcmc <- as.data.frame(fread(paste0("SSM_out/SSM_regression_MarSep_BLRP_muCommon_out_", 
  ...
  obs <- c(BLRP_MarSun$Mean_BLRP, BLRP_MarShade$Mean_BLRP, 
           BLRP_SepSun$Mean_BLRP, BLRP_SepShade$Mean_BLRP)
  for(j in 1:nrow(df_mcmc)){
    cor_mcmc_obs[j] <- cor(as.numeric(df_mcmc[j,use_idx]), obs[use_idx])
  }
  df_cor_rmse_loglik_BLRP[i,4:6] <- quantile99(cor_mcmc_obs)[2:4]
  df_cor_rmse_loglik_BLRP[i,7] <- mean(cor_mcmc_obs)
  ...
  df_cor_rmse_loglik_BLRP[i,8:10] <- quantile99(rmse_mcmc_obs)[2:4]
  df_cor_rmse_loglik_BLRP[i,11] <- mean(rmse_mcmc_obs)
  
  # Log likelihood
  df_log_lik <- as.data.frame(fread(paste0("SSM_out/SSM_regression_MarSep_BLRP_muCommon_out_", 
                                           "Lagtemp", Lag_env_BLRP$Lag_temp[i],
                                           "_Laglight", Lag_env_BLRP$Lag_light[i],
                                           "_LagSIG5", Lag_env_BLRP$Lag_SIG5[i],
                                           "_WAIC_rev2_loglik.csv")))
  df_log_lik <- df_log_lik %>% select_if(negate(anyNA))
  df_cor_rmse_loglik_BLRP[i,12:14] <- quantile99(rowMeans(df_log_lik))[2:4]
  df_cor_rmse_loglik_BLRP[i,15] <- mean(rowMeans(df_log_lik))
}


## Save output
save.image("data/cor_RMSE_loglik_MarSep_LagTempLight0_rev2.RData")
