...
## Load stan model
model <- cmdstan_model("stan_model/SSM_regression_MarSep_SIG5_muCommon_LagTempLight0_WAIC_rev2.stan")
...
for(i in 1:6){
  ...
  fit <- model$sample(
    ...
    sig_figs = 4,
    output_dir = "/tmp",
    output_basename = paste0("SIG5_WAIC_", i)
  )
  
  # 99% Bayesian credible intervals
  outcsv_name <- list.files("/tmp")
  outcsv_name <- outcsv_name[grep(paste0("SIG5_WAIC_", i), outcsv_name)]
  tmp_csv_b_temp <- NULL
  ...
  df_all <- rbind(df_b_temp, df_b_light, df_b_CCA1, df_alpha_MarSun, df_alpha_MarShade, df_alpha_SepSun, df_alpha_SepShade)
  df_all <- df_all %>%
    mutate(par = row.names(df_all)) %>%
    relocate(par)
  
  fwrite(df_all,
         file = paste0("SSM_out/SSM_regression_MarSep_SIG5_muCommon_out_", 
